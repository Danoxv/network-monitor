...
while (true) {
    ...
    echo 'Занято: ' . sizeConverter($memUsed, 'kilobyte') . ' из ' . sizeConverter($memTotal, 'kilobyte') . PHP_EOL;
    echo 'Процент занятости: ' . round($memUsage, 1) . '%' . PHP_EOL;

    echo 'Cвободно подкачки: ' . sizeConverter($swapFree, 'kilobyte') . PHP_EOL;
    echo 'Занято: ' . sizeConverter($swapUsed, 'kilobyte') . ' из ' . sizeConverter($swapTotal, 'kilobyte') . PHP_EOL;
